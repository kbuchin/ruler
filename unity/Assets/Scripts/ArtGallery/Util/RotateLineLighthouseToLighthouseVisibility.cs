...
{
    public static class RotateLineLighthouseToLighthouseVisibility
    {
            ...
            Vector2 vertex)
        {
            ...
            var polyPolarPoints = polyVertexes.Select(v => v - vertex).Select(x => new PolarPoint2D(x)).ToList();
            var visPolarPoints = visibilityVertexes.Select(v => v - vertex).Select(x => new PolarPoint2D(x)).ToList();
            ...
            while (!done)
            {
                ...
                var polyLast = polyVertexes[(polyIndex - 1 + polyCount) % polyCount];
                var visNext = visibilityVertexes[(visIndex+1)%visCount];
                var polyLineSegment = new LineSegment(polyCurrent, polyNext);
                var polyLineSegmentLast = new LineSegment(polyLast, polyCurrent);
