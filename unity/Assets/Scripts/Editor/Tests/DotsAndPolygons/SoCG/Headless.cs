...
namespace DotsAndPolygons.Tests.SoCG
{
    public class Headless
    {
        ...
        public void Test1()
        {
            ...
            List<int> numberOfDotsToTest = new List<int>() {3};
                                            ...
                                            for (int i = 0; i < 1; i++)
                                            {
                                                ...
                                                switch (mode)
                                                {
                                                    case GameMode.GameMode1:
                                                        controller = new HeadlessDotsController1(
                                                            new MinMaxAi(PlayerNumber.Player1, mode, deptsp1,
                                                                threshold1, numberOfThreadsp1),
                                                            new MinMaxAi(PlayerNumber.Player2, mode, deptsp2,
                                                                threshold2, numberOfThreadsp2), numberOfDots);
                                                        break;
                                                    case GameMode.GameMode2:
                                                        controller = new HeadlessDotsController2(
                                                            new MinMaxAi(PlayerNumber.Player1, mode, deptsp1,
                                                                threshold1, numberOfThreadsp1),
                                                            new MinMaxAi(PlayerNumber.Player2, mode, deptsp2,
                                                                threshold2, numberOfThreadsp2), numberOfDots);
            ...
            using (FileStream writer2 =
                new FileStream($"/mnt/SSD/Projects/TUe/ruler/testresults/results-{DateTime.Now:dd-MM-yyyy-HH.mm}.csv",
                    FileMode.Create, FileAccess.Write))
            {
                using (StreamWriter writer = new StreamWriter(writer2))
                {
                    writer.WriteLine(
                        "P1AItype;P2AIType;GameMode;NumberOfDots;P1NumerOfThreads;P2NumberOfThreads;P1AIDepth;P2AIDepth;P1Threshold;P2Threshold;P1AVGRunningTime;P2AVGRunningTime;P1FinalScore;P2FinalScore;Winner");
