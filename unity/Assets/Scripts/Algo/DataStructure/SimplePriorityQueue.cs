...
namespace Algo
{
    public sealed class SimplePriorityQueue<T> : IPriorityQueue<T>
    {
        ...
        /// <summary>
        /// Does the same as UpdatePriority() but only acts when the priority would be decreased.  
        /// O(n)
        /// </summary>
        public void UpdatePriorityDecreasing( T item, double newpriority)
        {
            lock (_queue)
            {
                try
                {
                    ...
                    _queue.UpdatePriorityDecreasing(updateMe, newpriority);
                }
                catch (InvalidOperationException ex)
                {
        ...
        /// <summary>
        /// Retrieves the lowest used priority (i.e in the head of the queue) in O(1) time
        /// </summary>
        /// <returns></returns>
        public double LowestPriority()
        {
            lock (_queue)
            {
