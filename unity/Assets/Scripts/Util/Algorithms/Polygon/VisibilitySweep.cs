    ...
    public class VisibilitySweep
    {
        ...
        public static Polygon2D Vision(Polygon2DWithHoles polygon, Vector2 x)
        {
            ...
            for (var i = 0; i < events.Count; i++)
            {
                ...
                bool insert1 = false, insert2 = false;
            ...
            var poly = Postprocess(visibility, x, initAngle);
            //Debug.Log(poly);
            return poly;
        }
        ...
        private static List<VisibilityEvent> Preprocess(Polygon2DWithHoles poly, Vector2 x, out float initAngle)
        {
            ...
            {
                xSeg = holeSegments.FirstOrDefault(seg => seg.IsOnSegment(x) && !seg.IsEndpoint(x));
                if (xSeg != null)
                {
                    var i = holeSegments.IndexOf(xSeg);
                    holeSegments.Insert(i + 1, new LineSegment(x, xSeg.Point2));
            ...
            {
                xSeg = holeSegments.FirstOrDefault(seg => MathUtil.EqualsEps(seg.Point1, x));
                if (xSeg != null)
                {
            ...
            if (rot)
            {
                ...
                holeSegments = holeSegments.Select(seg => new LineSegment(MathUtil.Rotate(seg.Point1, -angle),
                    MathUtil.Rotate(seg.Point2, -angle))).ToList();
            }
            ...
            events.AddRange(CreateEvents(holeSegments, true));
    ...
    public class StatusItem : IComparable<StatusItem>, IEquatable<StatusItem>
    {
        ...
        public int CompareTo(StatusItem other)
        {
            ...
            foreach (var p in points)
            {
                ...
                var p1 = seg.Intersect(ray);
                var p2 = seg2.Intersect(ray);

                if (p1.HasValue && p2.HasValue && !MathUtil.EqualsEps(p1.Value, p2.Value)) 
                {
