namespace Util.Algorithms.Polygon
{
    ...
    /// <summary>
    ///     Collection of algorithms related to visibility
    ///     polygons.
    /// </summary>
    public static class Visibility
    {
        /// <summary>
        ///     Computes the visibility polygon from the given point inside
        ///     of a simple polygon (given as n vertices in CCW order) in
        ///     O(n) time. Based on:
        ...
        public static Polygon2D Vision(Polygon2D polygon, Vector2 z)
        {
            ...
            if (!polygon.ContainsInside(z))
            {
                throw new ArgumentException(
                    "Visibility point must be inside polygon");
            }
        ...
        /// <summary>
        ///     Preprocess polygon such that z is at origin and vertices
        ///     are stored as polar points with angular displacements.
        ...
        /// <returns></returns>
        private static VsRep Preprocess(
            Polygon2D pol,
            Vector2 z,
            out double initAngle)
        {
            ...
            var vertices = pol.Vertices.Skip(vIndex)
                              .Concat(pol.Vertices.Take(vIndex))
                              .ToList();
        ...
        /// <summary>
        ///     Pushes a new vertex on the stack and calls the appropriated
        ///     function (advance, retard, scan) depending on the next
        ...
        /// <param name="i"></param>
        /// <returns></returns>
        public static NextCall Advance(
            ref VsRep v,
            ref Stack<VertDispl> s,
            ref int i,
            ref VertDispl w,
            ref bool ccw)
        {
            ...
            else
            {
                ...
                {
                    var isect = (new LineSegment(
                        v.Get(i).p.Cartesian,
                        v.Get(i + 1).p.Cartesian)).Intersect(v0.p.Ray);

                    Debug.Assert(isect != null);

                    var st = DisplacementInBetween(
                        new PolarPoint2D(isect.Value),
                        v.Get(i),
                        v.Get(i + 1));
        ...
        /// <summary>
        ///     Pops all vertices from the stack that have become invisible
        ///     after the addition of a new vertex. Calls appropriate
        ///     method (advance, scan, retard) after being done based on
        ///     next vertex.
        /// </summary>
        /// <param name="v"></param>
            ...
            ref bool ccw)
        {
            ...
            if (sj.alpha < v.Get(i + 1).alpha)
            {
                ...
                var p = (new LineSegment(sj.p.Cartesian, sjNext.p.Cartesian))
                    .Intersect(vi.p.Ray);
                ...
                if (p == null)
                {
                    var line = new Line(
                        vi.p.Ray.origin,
                        vi.p.Ray.origin + vi.p.Ray.direction);

                    p = (new LineSegment(sj.p.Cartesian, sjNext.p.Cartesian))
                        .Line.Intersect(line);

                    p = Vector2.Distance(p.Value, sj.p.Cartesian) <
                        Vector2.Distance(p.Value, sjNext.p.Cartesian)
                        ? sj.p.Cartesian
                        : sjNext.p.Cartesian;
                }

                var st1 = DisplacementInBetween(
                    new PolarPoint2D(p.Value),
                    sj,
                    sjNext);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (w == null)
                    {
                        ...
                        w = DisplacementInBetween(
                            new PolarPoint2D(res),
                            v.Get(i),
                            v.Get(i + 1));
                    }
        ...
        /// <summary>
        ///     Scans the vertices on the polygon until either an advance
        ///     or retard step can be made. Used when iterating over a
        ///     section of the polygon not visible from z.
        /// </summary>
        /// <param name="v"></param>
        /// <param name="s"></param>
        /// <param name="iprev"></param>
        ...
        /// <returns> </returns>
        public static NextCall Scan(
            ref VsRep v,
            ref Stack<VertDispl> s,
            ref int i,
            ref VertDispl windowEnd,
            ref bool ccw)
        {
            while (i < v.n)
            {
                ...
                {
                    VertDispl intersec = IntersectWithWindow(
                        v.Get(i),
                        v.Get(i + 1),
                        s.Peek(),
                        windowEnd);
        ...
        /// <summary>
        ///     Either intersects two line segments defined by the given
        ///     four polar points or if endpoint is null, a line segment
        ///     (a,b) and ray from point orig in the direction of its
        ///     adjacent polygon segment.
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        ...
        /// <returns>
        ///     A vertex-displacement pair associated with the
        ///     intersection.
        /// </returns>
        public static VertDispl IntersectWithWindow(
            VertDispl a,
            VertDispl b,
            VertDispl orig,
            VertDispl endpoint)
        ...
        ///     Increments/decrements a(s) with 2 * PI until between a(v1)
        ///     and a(v2).
        /// </summary>
        /// <param name="s"></param>
        /// <param name="v1"></param>
        ...
        public static VertDispl DisplacementInBetween(
            PolarPoint2D s,
            VertDispl v1,
            VertDispl v2)
        {
        ...
        public static VertDispl LocateSj(
            VertDispl vi,
            VertDispl vi1,
            Stack<VertDispl> ss)
        {
            ...
            while (ss.Count > 0)
            {
                ...
                if (MathUtil.LEQEps(sj.alpha, vi1.alpha) &&
                    MathUtil.LEQEps(vi1.alpha, sj1.alpha))
                {
                    ...
                    MathUtil.EqualsEps(sj.alpha, sj1.alpha))
                {
                    var y = (new LineSegment(vi.p.Cartesian, vi1.p.Cartesian))
                        .Intersect(
                            new LineSegment(sj.p.Cartesian, sj1.p.Cartesian));
        ...
        /// <summary>
        ///     Transforms the vertices on the stack to the final
        ///     visibility polygon. Reverses the pre-process steps, namely
        ///     the translation and rotation. Also removes one of endpoints
        ///     if duplicate
        /// </summary>
        ...
        private static Polygon2D Postprocess(
            List<VertDispl> pre_s,
            VsRep vs,
            Vector2 z,
            double initAngle)
        {
            ...
            if (shiftedPol.Count > 1 &&
                MathUtil.EqualsEps(
                    shiftedPol.FirstOrDefault(),
                    shiftedPol.LastOrDefault(),
                    MathUtil.EPS * 10))
            {
        ...
        /// <summary>
        ///     Computes the angular displacement of each polar vertex with
        ///     respect to starting vertex. As discussed in paper, the
        ///     angular displacement of vertex is the previous displacement
        ///     with either + angle or - angle based on whether the current
        ///     vertex constitutes a left or right turn. (or no
        ///     addition/subtraction if the angle is exactly PI)
        /// </summary>
        /// <param name="v"></param>
        /// <returns>A list of vertex-displacement pairs. </returns>
        private static List<VertDispl> ComputeAngularDisplacements(
            List<PolarPoint2D> v)
        {
            ...
            for (var i = 0; i < v.Count; i++)
            {
                ...
                Debug.Assert(
                    MathUtil.LEQEps(
                        Math.Abs(alpha_vi - ret[i - 1].alpha),
                        MathUtil.PI));
        ...
        private static Vector2 GetInitialVertex(
            Polygon2D shiftedPol,
            bool zIsVertex,
            out int index)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < segments.Count; i++)
                {
                    ...
                    if (Math.Sign(curr.Point1.y) == Math.Sign(curr.Point2.y))
                        continue;
                    ...
                    if (intersection.HasValue &&
                        (!closestVisibleVertex.HasValue ||
                         intersection.Value.x < closestVisibleVertex.Value.x))
                    {
                        index = (i + 1) % segments.Count;
        ...
        /// <summary>
        ///     Stores a vertex as a polar point and an angular
        ///     displacement.
        /// </summary>
        public class VertDispl
        {
            ...
            /// <summary>The direction vector of the angular displacement.</summary>
        ...
        /// <summary>
        ///     Representation structure for the vertices with angular
        ///     displacements. Plus some additional data like whether z is
        ///     a vertex in the polygon.
        /// </summary>
        public class VsRep
        {
            ...
            /// <summary>Gets the vertex+displacement at given position.</summary>
            /// <param name="i"></param>
            /// <returns></returns>
